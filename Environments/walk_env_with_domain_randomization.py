...
class DomainRandomization:
    ...
    def randomize(self, env_idx = None):
        ...
        self.robot.set_friction_ratio(
            friction_ratio=0.7 + torch.rand(total_envs, self.robot.n_links) * 0.6,
        ...
        kp_values = torch.clamp(20 + 10* torch.randn(12), 5, 50)
        self.robot.set_dofs_kp(kp_values, self.joints_local_idx, envs_idx = randomize_envs)

        kv_values = torch.clamp(0.5 + torch.randn(12), 0.2, 20)
        self.robot.set_dofs_kv(kv_values, self.joints_local_idx, envs_idx = randomize_envs)



class WalkENV(gym.Env):

    def __init__(self, render = True, backend = gs.gpu, n_batch = 100, morph = None, surface = None):
        super().__init__()
        ...
        # terrain = self.scene.add_entity(morph= morph)
        # terrain = self.scene.add_entity(morph = morph, surface= surface)
        ...
        self.domainrandomizer = DomainRandomization(self.robot, self.batch, self.joints_local_idx)
    ...
    def reset_env_idx(self, envs_ids):
        ...
        self.domainrandomizer.randomize(envs_ids)
    ...
    def reset(self, *, seed=None, options=None):
        ...
        all_envs = torch.arange(self.batch, device=self.device)
        self.reset_env_idx(all_envs)
